...
If a board does not include external RAM, then asserting CG should result in it flipping back to a
value of zero. Platform firmware is to assume embedded mode if it detects that CG will not stick to
a value of one.

## Special registers

Special registers are control registers used to configure the current processor. They are identified with a numeric ID
that is stored in the G1 register during read/write operations. The value returned by or fed into the read/write operation
is stored in the G0 register. To read a special register, the SRR instruction is used. To write a special register, the SRW
instruction is used.

```
ID            NAME          DESCRIPTION
-------------------------------------------------------
0x00000000   Reserved       Reserved, must be unused
0x00000001   IntConf        Interrupt configuration
-------------------------------------------------------
```

### Interrupt configuration register

The interrupt configuration register allows system software to configure interrupt related
such as the masking of synchronous interrupts, etc.

```
BITS       NAME                 PURPOSE
----------------------------------------------------------------
0       Interrupt mask      Interrupts are masked when set
8:1     Interrupt priority  Interrupt priority for current PD
63:9    Reserved            Reserved for future use
---------------------------------------------------------------
```
